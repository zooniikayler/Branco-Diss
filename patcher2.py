...
import sys
import mappy as mp
from process_polyTE import filter_fastq, run_polyte
import alignment2
import multiproc2
...
def polyte_steps(reffile, reads1, reads2, int_fname, fname, output_type, TE):
    filter_fastq(TE, reads1, reads2, int_fname)
    run_polyte(reffile, int_fname, fname, output_type)
...
def main(argv):
    ...
    if '-g' in myargs:
        ...
        print_help()
    if '-o' in myargs:
        fname = myargs["-o"]/var/folders/sm/tmsr4vt95wsbb57yrr6szm9w0000gn/T/com.apple.iChat/Messages/Transfers/IMG_4777.PNG
    else:
        print_help()
    if '-r1' in myargs:
        r1name = myargs["-r1"]
    ...
    if nthreads > 3:
        multiproc2.run(reffile, reads1, reads2, fname,
