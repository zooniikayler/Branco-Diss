...
class SAMBAMWriter():
    ...
    def process_te_output(self, res, read1):
        """
        Take output from alignment (res) and read1
        and process them to SAM/BAM
        """
        ...
        for x in range(len(res)):
            if res[x]['strand'] == -1:
                flag += 16
            # There is a bug in mappy that clips the mapping but doesn't provide the right cigar
            read1.seq = read1.seq[res[x]["q_st"]:res[x]["q_en"]]
            read1.qual = read1.qual[res[x]["q_st"]:res[x]["q_en"]]
            self.write_read(read1, res[x], flag, "s")
